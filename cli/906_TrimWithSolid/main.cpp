...
{
  argparse::ArgumentParser parser("906_TrimWithSolid", "1.0");
  parser.add_argument("-i", "--input")
    .required()
    .help("path to input mesh");
  parser.add_argument("-i2", "--input2")
    .required()
    .help("path to solid mesh");
  ...
  std::string solid_path = parser.get<std::string>("--input2");
  ...
  igl::read_triangle_mesh(mesh_path, VA, FA);
  // Load a solid mesh
  igl::read_triangle_mesh(solid_path, VB, FB);
  // resize bunny to fit properly
