...
{
  argparse::ArgumentParser parser("904_FastFindIntersections", "1.0");
  parser.add_argument("-i1", "--input1")
    .required()
    .help("path to first input mesh");
  parser.add_argument("-i2", "--input2")
    .required()
    .help("path to second input mesh");
  ...
  std::string mesh_path1 = parser.get<std::string>("--input1");
  std::string mesh_path2 = parser.get<std::string>("--input2");

  // Load two meshes 
  igl::read_triangle_mesh(mesh_path1, V1, F1);
  igl::read_triangle_mesh(mesh_path2, V2, F2);
