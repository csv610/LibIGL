...
{
  argparse::ArgumentParser program("403_BoundedBiharmonicWeights");
  program.add_argument("-i", "--input").required().help("Path to mesh file");
  program.add_argument("--tgf").required().help("Path to tgf file");
  program.add_argument("--pose").required().help("Path to pose dmat file");
  ...
  using namespace Eigen;
  using namespace std;
  igl::readMESH(program.get<std::string>("--input"),V,T,F);
  U=V;
  ...
  MatrixXd Q;
  igl::readDMAT(program.get<std::string>("--pose"),Q);
