...
{
  IGL_TICTOC_LAMBDA;
  argparse::ArgumentParser parser("SplineDistance", "1.0");
  parser.add_argument("--points")
    .required()
    .help("control points DMAT file");
  parser.add_argument("--controls")
    .required()
    .help("control indices DMAT file");
  try {
  ...
  std::string points_path = parser.get<std::string>("--points");
  std::string controls_path = parser.get<std::string>("--controls");

  Eigen::MatrixXd P;
  Eigen::MatrixXi C;
  igl::readDMAT(points_path, P);
  igl::readDMAT(controls_path, C);
