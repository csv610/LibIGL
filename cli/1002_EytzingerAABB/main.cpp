...
{
  IGL_TICTOC_LAMBDA;
  argparse::ArgumentParser parser("EytzingerAABB", "1.0");
  parser.add_argument("-i", "--input")
    .required()
    .help("input mesh");
  parser.add_argument("--radii")
    .required()
    .help("radii DMAT file (optional)");
  try {
  ...
  std::string radii_path = parser.get<std::string>("--radii");
