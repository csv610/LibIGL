...
{
  argparse::ArgumentParser program("407_BiharmonicCoordinates");
  program.add_argument("-i1", "--input1").required().help("Path to low res mesh file");
  program.add_argument("-i2", "--input2").required().help("Path to high res mesh file");
  ...
  if(!readMESH(program.get<std::string>("--input1"),low.V,low.T,low.F))
  {
    cout<<"failed to load mesh"<<endl;
  }
  if(!readMESH(program.get<std::string>("--input2"),high.V,high.T,high.F))
  {
    cout<<"failed to load mesh"<<endl;
  }
