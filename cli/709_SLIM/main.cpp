...
void soft_const_demo_iter(igl::opengl::glfw::Viewer& viewer) {
  ...
  if (first_iter) {
    ...
    igl::read_triangle_mesh(std::string(TUTORIAL_SHARED_PATH) + "/circle.obj", V, F);
...
void deform_3d_demo_iter(igl::opengl::glfw::Viewer& viewer) {
  ...
  if (first_iter) {
    timer.start();
    igl::read_triangle_mesh(TUTORIAL_SHARED_PATH "/cube_40k.obj", V, F);
...
int main(int argc, char *argv[]) {
  ...
  argparse::ArgumentParser program("709_SLIM");
  program.add_argument("-i", "--input").required().help("Path to mesh file");
  program.add_argument("demo").default_value("1").help("Demo number (1 to 3)");
  ...
  std::string demo_arg = program.get<std::string>("demo");
  if (!demo_arg.empty()) {
    switch (std::atoi(demo_arg.c_str())) {
      case 1: {
        demo_type = PARAM_2D;
        break;
      } case 2: {
