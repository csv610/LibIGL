...
{
  argparse::ArgumentParser program("401_BiharmonicDeformation");
  program.add_argument("-i", "--input").required().help("Path to mesh file");
  program.add_argument("--selection").required().help("Path to selection file");
  ...
  VectorXi S;
  igl::readDMAT(program.get<std::string>("--selection"),S);
