...
{
  IGL_TICTOC_LAMBDA;
  argparse::ArgumentParser parser("WindingNumber2D", "1.0");
  parser.add_argument("--vertices")
    .required()
    .help("vertices DMAT file");
  parser.add_argument("--edges")
    .required()
    .help("edges DMAT file");
  try {
  ...
  std::string vertices_path = parser.get<std::string>("--vertices");
  std::string edges_path = parser.get<std::string>("--edges");

  Eigen::MatrixXd V;
  Eigen::MatrixXi E;
  igl::readDMAT(vertices_path, V);
  igl::readDMAT(edges_path, E);
